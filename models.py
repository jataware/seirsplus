...
# Convert baseline NOLH values to scaled input parameters based on init.json "DOE"
def NOLH(x, lo, hi, dc, numRuns):
    return round(lo + ((x-1)*(hi-lo)/numRuns),dc)

def buildNOLH(df_base, params, numRuns):
    ...
    for key in params.keys():
        ...
        df[key] = df[key].apply(lambda x: NOLH(x, lo, hi, dc, numRuns))
        
    return df

def shape_nolh(df_base, params):
   
    dropCol = len(df_base.columns) - len(params)
    if dropCol > 0:
        df_base.drop(df_base.columns[-dropCol:], axis=1, inplace=True)

    #...and rename to column headers to param keys
    names  = pd.DataFrame(params).columns.to_series()
    df_base.columns = names
   
    return df_base

# Block output into 10s because dt=0.1 in models.py script (10 iterations / day) [code lines 130 and 173]
def timeseries_it(model, attr, days, startDate):
...
def casesMSE(df):
    ...
    return np.sqrt(mean_squared_error(testCases, predCases))

def fatalitiesMSE(df):
    testF = df['actualFatalities']
    predF = df['predictedFatalities']
    return np.sqrt(mean_squared_error(testF, predF))
    ...
    #def run(self, T, dt=1, checkpoints=None, verbose=False):
        ...
        if(not checkpoints):
            ...
            for checkpointIdx, checkpointTime in enumerate(checkpoints['t']):
                ...
                #print("[Checkpoint: Updating parameters]",end="\r")
                for param in paramNames:
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None,  plot_S='line', plot_E='line', plot_I='line',plot_R='line', plot_F='line',
                            ...
                            ylim=None, xlim=None, legend=True, title=None, side_title=None, plot_percentages=plot_perc):
        ...
        if(legend):
            ...
            ax.legend(legend_handles[::-1], legend_labels[::-1], loc='upper right', facecolor='white', edgecolor='none', framealpha=0.9, prop={'size': 18})
        if(title):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def figure_basic(self, plot_S='line', plot_E='line', plot_I='line',plot_R='line', plot_F='line',
                        ...
                        vlines=[], vline_colors=[], vline_styles=[], vline_labels=[],
                        ylim=None, xlim=None, legend=True, title=None, side_title=None, plot_percentages=plot_perc,
                        figsize=(20,10), use_seaborn=True, show=True):
